...
async function attachFile(bridge, filePath) {
    ...
    if (!fileInput?.nodeId)
        throw new Error('Could not find file input #upload-files');
...
export const promodeCommand = cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!cdpBridge)
            throw new Error('CDP bridge not accessible — only works with OPENCLI_CDP_ENDPOINT');
        ...
        const inputRef = snapshot?.nodes?.find((n) => n.role === 'textbox' && n.id === 'prompt-textarea')?.ref ??
            ...
            if (messagesNow.length > messagesBefore.length) {
                ...
                if (candidate && candidate.length > 50) {
                    ...
                    for (let i = 0; i < 720; i++) { // up to 1 hour after first response
                        ...
                        const isThinking = cur.startsWith('Pro thinking') || cur.startsWith('Thinking') ||
