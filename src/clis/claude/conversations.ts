...
export const conversationsCommand = cli({
  site: 'claude',
  name: 'conversations',
  description: 'List Claude.ai conversations from sidebar; optionally navigate to one by index',
  domain: 'claude.ai',
  ...
  func: async (page: IPage | null, kwargs: any) => {
    ...
    const raw = await page.evaluate(`
      (function() {
        const links = Array.from(document.querySelectorAll('a[href*="/chat/"], nav a[href*="/c/"]'));
        const seen = new Set();
        const convs = [];
    ...
    if (!convs.length) return [{ '#': '-', Title: 'No conversations found', Active: '' }];
    ...
    if (idx !== null) {
      ...
      const url = conv.href.startsWith('http') ? conv.href : `https://claude.ai${conv.href}`;
      await page.goto(url);
      await page.wait(2);
