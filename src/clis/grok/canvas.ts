...
export const canvasCommand = cli({
  site: 'grok',
  name: 'canvas',
  description: 'View or interact with Grok canvas/artifact content',
  domain: 'grok.com',
  ...
  func: async (page: IPage | null, kwargs: any) => {
    ...
    const content = await page.evaluate(`
      (function() {
        const canvas = document.querySelector('[class*="canvas"], [class*="Canvas"], [data-canvas], .artifact');
        if (canvas) {
        ...
        if (last) {
          ...
          return JSON.stringify({ title: 'Last Response', content: (last.innerText || '').trim().slice(0, 3000) });
        }
