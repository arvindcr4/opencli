...
export const conversationsCommand = cli({
  site: 'grok',
  name: 'conversations',
  description: 'List Grok conversations; optionally navigate to one by index',
  domain: 'grok.com',
  ...
  func: async (page: IPage | null, kwargs: any) => {
    ...
    const raw = await page.evaluate(`
      (function() {
        const links = Array.from(document.querySelectorAll(
          'nav a[href*="/chat/"], aside a[href*="/chat/"], a[href*="/conversation/"]'
        ));
        const seen = new Set();
        const convs = [];
    ...
    if (idx !== null) {
      ...
      const url = conv.href.startsWith('http') ? conv.href : `https://grok.com${conv.href}`;
      await page.goto(url);
      await page.wait(2);
