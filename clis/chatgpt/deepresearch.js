...
async function getDeepResearchReport(page) {
  ...
  return result.map((s) => s.replace(/[\uFFFC\u200B-\u200D\uFEFF]/g, "").trim()).filter((s) => s.length > 0);
}
async function isResearching(page) {
  const result = await page.evaluate(`
    (function() {
      ...
      if (/searchings+d+/i.test(document.body.innerText || '')) return true;
      return false;
    })()
  `);
  return !!result;
...
const deepresearchCommand = cli({
  ...
  func: async (page, kwargs) => {
    ...
    const timeout = parseInt(kwargs.timeout, 10) || 900;
    if (!page) throw new Error("Browser page not available");
    await page.goto("https://chatgpt.com/deep-research");
    await page.wait(3);
    const snapshot = await page.snapshot({ interactive: true });
    const inputRef = snapshot?.nodes?.find((n) => n.role === "textbox" && n.id === "prompt-textarea")?.ref ?? snapshot?.nodes?.find((n) => n.role === "textbox" && n.placeholder?.toLowerCase().includes("report"))?.ref ?? snapshot?.nodes?.find((n) => n.role === "textbox" && n.name?.includes("prompt"))?.ref;
    if (!inputRef)
